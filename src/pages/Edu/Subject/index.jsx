...
class Subject extends Component {
  ...
  state = {
    // subjectId的作用
    // 1.如果subjectId没有表示表格每一行直接展示课程分类的title,如果有值
    //   (应该就是要修改数据的id)
    // 2.修改数据需要subjectid
    subjectId: "",
  ...
  handleUpdate = async () => {
    ...
    // 思路:点击更新按钮的时候,把旧的课程分类名称存起来,点击确认的时候,用新数据(subjectTitle) 和老数据进行比较
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        // 自定义这一列要渲染的内容
        render: (value) => {
          // 判断当前数据的id是否和state里面subjectId的值是相同的,如果相同,展示
          // 示确认和取消按钮,否则展示修改和删除按钮
          if (this.state.subjectId === value._id) {
            return (
              <>
                <Button
          ...
          return (
            <>
              ...
              <Tooltip title="删除课程分类" onClick={this.handleDel(value)}>
                <Button type="danger">
                  <DeleteOutlined />
                </Button>
              </Tooltip>
